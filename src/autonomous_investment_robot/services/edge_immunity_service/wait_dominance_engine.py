...
def evaluate_wait_dominance(*, base_edge_bps: float, stressed_edge_bps: float, wait_bonus_bps: float, fragility_index: float) -> WaitDominanceReport:
    ...
    incremental_advantage = max(0.0, wait_score - trade_now_score)
    wait_dominant = wait_score > trade_now_score
    return WaitDominanceReport(
        wait_value_score=max(0.0, wait_bonus_bps),
        ...
        metadata={
            ...
            "wait_bonus_bps": wait_bonus_bps,
            "wait_score_bps": wait_score,
            "incremental_advantage_bps": incremental_advantage,
        },
    )

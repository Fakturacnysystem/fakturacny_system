...
def _summarize_row(config_path: Path, settings: RobotSettings, resolved: dict[str, Any], status: str, error: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "distributed_enabled": bool(getattr(settings.distributed, "enabled", False)),
        "distributed_node_role": str(getattr(settings.distributed, "node_role", "")),
        "distributed_compute_bridge": str(getattr(settings.distributed, "compute_bridge", "")),
        "distributed_stream_prefix": str(getattr(settings.distributed, "stream_prefix", "")),
        "postgres_mirror_enabled": bool(getattr(settings.distributed, "postgres_mirror_enabled", False)),
